...
from converters import toNumpyArray, toTorchImage, toCVShape
...
class TorchNeuralNet:
    ...
    def forward(self, rgbImg):
        ...
        self._tensor[0] = torch.fromNumpyArray(toTorchImage(rgbImg))
    ...
    def backward(self, rgbImg, grad):
        """
        Perform a backward network pass of an RGB image.

        :param rgbImg: RGB image to process. Shape: (imgDim, imgDim, 3)
        :type rgbImg: numpy.ndarray
        :param grad: Vector of loss gradient
        ...
        self._tensor[0] = torch.fromNumpyArray(toTorchImage(rgbImg))
        self._tensor[0] = image.scale(self._tensor[0], self._imgDim, self._imgDim)
        self._grad_tensor = torch.fromNumpyArray(np.array(grad, dtype=np.float32))
        self._cuda_grad_tensor = None
        if self._cuda:
            lua.require('cutorch')
            lua.require('cunn')
            self._cuda_tensor._copy(self._tensor)
            self._cuda_grad_tensor = torch.CudaTensor(1, 128)
            self._cuda_grad_tensor._copy(self._grad_tensor)
            gradInput = self._net._updateGradInput(self._cuda_tensor, self._cuda_grad_tensor)._float()
        else:
            gradInput = self._net._updateGradInput(self._tensor, self._grad_tensor)
        return toCVShape(toNumpyArray(gradInput[0]))

    @staticmethod
    def loadImage(path):
        return toNumpyArray(image.load(path, 3, 'float'))
